...
exercise2 = Exercise.create(user:user, warmup:true, name:"Tree, Nut, Squirrel", description:"tbd")
Tag.create(name:"Warmup", exercise_id:exercise2.id)

exercise3 = Exercise.create(user:user, warmup:true, name:"Unison Clap", description:"Go around the circle each pair of two clap in unison. So you receive someone about to clap, then turn and clap in unison with the person beside you. You can also clap twice, and send it the other direction. The person who turns and claps needs to be aware that the other person might hold that position and want to clap again. Then you clap across the circle. Then you walk around and still clap with the people who were next to you.")
Tag.create(name:"Warmup", exercise_id:exercise3.id)

exercise4 = Exercise.create(user:user, warmup:true, name:"Bad Rap", description:"tbd")
Tag.create(name:"Warmup", exercise_id:exercise4.id)

exercise5 = Exercise.create(user:user, warmup:true, name:"Beastie Rap", description:"tbd")
Tag.create(name:"Warmup", exercise_id:exercise5.id)

exercise6 = Exercise.create(user:user, warmup:true, name:"Hey Fred Schneider", description:"tbd")
Tag.create(name:"Warmup", exercise_id:exercise6.id)

exercise7 = Exercise.create(user:user, warmup:true, name:"Beat My Meat", description:"tbd")
Tag.create(name:"Warmup", exercise_id:exercise7.id)

exercise8 = Exercise.create(user:user, warmup:true, name:"Hot Spot", description:"tbd")
Tag.create(name:"Warmup", exercise_id:exercise8.id)

exercise12 = Exercise.create(user:user, warmup:true, name:"Bunny Bunny", description:"tbd")
Tag.create(name:"Warmup", exercise_id:exercise12.id)

exercise13 = Exercise.create(user:user, warmup:true, name:"Zip Zap Zop", description:"tbd")
Tag.create(name:"Warmup", exercise_id:exercise13.id)

exercise14 = Exercise.create(user:user, warmup:true, name:"Dukes of Hazzard", description:"tbd")
Tag.create(name:"Warmup", exercise_id:exercise14.id)

exercise15 = Exercise.create(user:user, warmup:true, name:"Whoosh/Bang/Ramp/Interstate", description:"tbd")
Tag.create(name:"Warmup", exercise_id:exercise15.id)

exercise16 = Exercise.create(user:user, warmup:true, name:"Four Corners", description:"A volunteer stands in one corner of the room and makes a declarative statement that is true about themselves. (Ex. 'I've seen all the Harry Potter movies' or 'I've lived outside of the U.S.') Then everyone of whom that is also true moves to that corner and everyone who that is not true moves to another corner.  Then another volunteer stands in a new corner and makes another statement that is true about themselves, etc.")
Tag.create(name:"Warmup", exercise_id:exercise16.id)

exercise17 = Exercise.create(user:user, warmup:true, name:"Mr. Whiskers", description:"Mr. Whiskers goes across the circle, Misty Vistas is passed to the left, and Whiskey Mixers is passed to the right. If someone laughs, they have to run around the circle.")
Tag.create(name:"Warmup", exercise_id:exercise17.id)

exercise18 = Exercise.create(user:user, warmup:true, name:"Bididit / Electric Company", description:"The class stands in a circle and hits their legs softly to create a rhythm. First person says a word, next person responds with another word on the beat, then the entire class says both words in order on the beat followed with 'Bididit'. Great way to get people to just respond to information.")
Tag.create(name:"Warmup", exercise_id:exercise18.id)

exercise20 = Exercise.create(user:user, warmup:true, name:"Zen Count", description:"tbd")
Tag.create(name:"Warmup", exercise_id:exercise20.id)

exercise21 = Exercise.create(user:user, warmup:true, name:"Bowl of Spaghetti", description:"tbd")
Tag.create(name:"Warmup", exercise_id:exercise21.id)

exercise22 = Exercise.create(user:user, warmup:true, name:"Knife Throw / Die, Not Today!", description:"One person starts throwing a ‘knife' at someone across the circle. Try and sell the weight of the knife (most people forget this). You will ‘catch' it by slapping together your hands in front of your face like an action hero. The teacher can add more knives to the group once they get the hang of it. At any point you can say ‘stop!' and ask everyone to hold up their invisible knives. See how many there are.")
Tag.create(name:"Warmup", exercise_id:exercise22.id)
Tag.create(name:"Energy", exercise_id:exercise22.id)

exercise23 = Exercise.create(user:user, warmup:true, name:"Samurai Sword Chop", description:"tbd")
Tag.create(name:"Warmup", exercise_id:exercise23.id)

exercise24 = Exercise.create(user:user, warmup:true, name:"Sound And Motion", description:"Being neutral and ready to play. Honoring through mirroring and heightening. What did we observe? Tendencies (group or personal)? What patterns did we see emerge? This exercise always helps people see how easy it is connect with one another in a non-judgemental way. You might feel silly at first, but trusting the group is so much more fun.")
...
exercise36 = Exercise.create(user:user, warmup:true, name:"Why Were You Late", description:" Students will describe why they were late using a movie plot. Whoever guesses the movie goes next. If someone guesses who has already gone, they choose who gets to say why they were late.")
Tag.create(name:"Warmup", exercise_id:exercise36.id)

exercise37 = Exercise.create(user:user, warmup:true, name:"What's on Your Busted Tee?", description:"tbd")
Tag.create(name:"Warmup", exercise_id:exercise37.id)
#
# exercise34 = Exercise.create(user:user, warmup:true, name:"", description:"")
# Tag.create(name:"Warmup", exercise_id:exercise34.id)
...
km_exercise17 = Exercise.create(user:user, warmup:false, name:"No Gap Dialog", description:"Two people begin a scene with a shared everyday activity. One of the players makes a statement about the situation. Both players attempt to eliminate all pauses or gaps between lines of dialog. This doesn't mean that they have to speak fast or that they cannot have a pause within a line of dialog. But when one player responds to the other, there should be no gap at all between the lines. Try to respond without thinking. Also, keep your lines to one or two sentences. Be sure to take turns. The scene continues until the coach edits it (about 45 - 90 seconds).")
...
km_exercise18 = Exercise.create(user:user, warmup:false, name:"Spontaneous Story", description:"Two person scene. When sidecoached to tell a story, one of the players should launch into a spontaneous story, by beginning with 'That reminds me of a story...' The story should feel like a non-sequitur at first and the player should press forward in the story without knowing where it is going. After the story is concluded, both players should examine the story to see if they can connect it to the rest of the scene, justifying why the story was told.")
...
km_exercise19 = Exercise.create(user:user, warmup:false, name:"Because of this", description:"Tell a story with about a dozen statements. Between each statement say the words, 'because of this' and make sure that each new moment of the story is caused by something that happens in the previous statement. Try not to skip steps, but tell the next direct thing that happened. Be specific in your details. Repeat several times as necessary.")
Tag.create(name:"Kevin Mullaney", exercise_id:km_exercise19.id)




# km_exercise18 = Exercise.create(user:user, warmup:false, name:"", description:"")
# Tag.create(name:"Kevin Mullaney", exercise_id:km_exercise18.id)
